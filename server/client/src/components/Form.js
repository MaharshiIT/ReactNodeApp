...
export function checkFileValidity(target,validExtensions,maxSize)
    ...
    file_object = target.files[0],
    {name,size} = file_object,  
    ...
    if (validExtensions && validExtensions.indexOf(extension) === -1)
        errorMessage = "Invalid File";  
    else if (maxSize && size_in_mb > maxSize)
        errorMessage = `File shouldn't be greater than ${maxSize} MB`;
    else valid = true;
...
function formField(WrappedComponent,errorMessage)
    {
     return class extends PureComponent
        ...
        getInitialState()
            {
            const initial_state = {borderClass:"",inValidClass:"hide"};
...
export class RadioButtons extends PureComponent
    ...
    render()
        {
        const {type,className,radio_labels,name,value,...remaining} = this.props; 
        return (
...
export class FileUpload extends PureComponent
    ...
    render()
        ...
        {formRef,isImage,className,onChange,...remaining} = this.props,

        imagePreview = (isImage) ? <img alt="" className="img-thumbnail image-preview" src={this.state.imageUrl}/> : "";
